<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
</head>
<body>
    
    <h1>BANGUN DATAR</h1>
    <ul>
        <li><a href="belah_ketupat copy.php">Belah ketupat</a></li>
        <li><a href="jajargenjang copy.php">Jajar Genjang</a></li>
        <li><a href="persegi_panjang copy.php">Persegi panjang</a></li>
        <li><a href="persegi copy.php">Persegi</a></li>
        <li><a href="lingkaran copy.php">Lingkaran</a></li>
        <li><a href="trapesium copy.php">Trapesium</a></li>
        <li><a href="layang_layang copy.php">Layang-layang</a></li>
        <li><a href="segitiga copy.php">Segitiga</a></li>
    </ul>
</body>
</html>